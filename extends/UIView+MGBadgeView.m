...
- (void)mg_updateBadgeViewPosition {
    ...
    switch (position) {
        ...
        case MGBadgePositionCenter: {
            self.center = CGPointMake(superviewFrame.size.width/2, superviewFrame.size.height/2);
            break;
        }
        default:
            break;
...
{
    UIImageView *checkMarkView = objc_getAssociatedObject(self, @selector(checkMarkView));
    ...
    if( !checkMarkView )
    {
        ...
        //[self.checkMarkView setCenter:self.center];
        checkMarkView = objc_getAssociatedObject(self, @selector(checkMarkView));
...
- (void)setCheckMarkView:(UIImageView *)checkMarkView
{
    objc_setAssociatedObject(self, @selector(checkMarkView), checkMarkView, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}


- (UIActivityIndicatorView *)progressView
{
    UIActivityIndicatorView *progressView = objc_getAssociatedObject(self, @selector(progressView));
    
    if( !progressView )
    {
        self.progressView = [[UIActivityIndicatorView alloc] initWithActivityIndicatorStyle:UIActivityIndicatorViewStyleWhiteLarge];
        [self.progressView setBackgroundColor:[UIColor colorWithRed:0.4f green:0.4f blue:0.4f alpha:0.5f]];
        [self.progressView setFrame:CGRectMake(0, 0, self.frame.size.width, self.frame.size.height)];
        progressView = objc_getAssociatedObject(self, @selector(progressView));
        [self addSubview:progressView];
    }
    
    return progressView;
}
- (void)setProgressView:(UIActivityIndicatorView *)progressView
{
    objc_setAssociatedObject(self, @selector(progressView), progressView, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}


- (void)showCheckMark:(BOOL)isShown
{
    [self.checkMarkView setHidden:!isShown];
}

- (void)showProgressView:(BOOL)isShown
{
    if( isShown )
